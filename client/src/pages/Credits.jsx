...
const Credits = () => {
   ...
   const purchasePlan = async (planId) => {
    try {
      const {data} = await axios.post('/api/credit/purchased', {planId}, 
        {header: {Authorization: token}})
        if(data.success){
  ...
  return (
    ...
    lg:px-8 py-12'>
      <h1 className='text-3xl font-medium text-center mb-10 xl:mt-30 
      text-gray-800 dark:text-white'>credit Plans</h1>
         ...
         {plans.map((plan)=>(
          ...
          ${plan._id === "pro" ? 'bg-purple-50 dark:bg-purple-900': 'bg-white dark:bg-transparent'}`}>
            ...
            <div className='flex-1'>
              <h1 className='text-xl font-semibold text-gray-500 dark:text-white mb-2'>
                ${plan.name}
              </h1>
            ...
            <button onClick={()=> toast.promise(purchasePlan(plan._id), {loading: 'loading......'})}
            className='mt-6 bg-purple-600 hover:bg-purple-800 active:bg-purple-600 
