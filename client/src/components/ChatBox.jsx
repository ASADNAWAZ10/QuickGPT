...
const ChatBox = () => {
  ...
  const onsubmit = async (e) => {
   try {
    ...
    setPrompt('')
    setMessage(prev => [...prev, {role: 'user', content:prompt, timestamps: Date.now(),
       isImage: false}])
  ...
  return (
   ...
   2xl:pr-40 '>
    ...
    <div ref={containerRef} className='flex-1 mb-5 overflow-y-scroll'>
       {message?.length === 0 && (
        <div>
         ...
         className='w-full max-w-56 sm:max-w-68' alt='logo'/>
    ...
    gap-4 items-center'>
          ...
          type="text"  placeholder='Type your prompt here....' 
          className='flex-1 w-full text-sm outline-none' required/>

          <button>
            <img src={loading ? assets.stop : assets.sendicon} alt="" 
